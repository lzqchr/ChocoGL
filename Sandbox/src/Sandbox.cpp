...
class EditorLayer : public ChocoGL::Layer
{
public:
	EditorLayer()
	...
	virtual void OnAttach() override
	{
		...
		m_Mesh.reset(new ChocoGL::Mesh("assets/models/m1911/m1911.fbx"));
		m_MeshMaterial.reset(new ChocoGL::MaterialInstance(m_Mesh->GetMaterial()));

		m_QuadShader = ChocoGL::Shader::Create("assets/shaders/quad.glsl");
		m_HDRShader = ChocoGL::Shader::Create("assets/shaders/hdr.glsl");

		m_SphereMesh.reset(new ChocoGL::Mesh("assets/models/Sphere1m.fbx"));
		m_PlaneMesh.reset(new ChocoGL::Mesh("assets/models/Plane1m.obj"));

		m_GridShader = ChocoGL::Shader::Create("assets/shaders/Grid.glsl");
		m_GridMaterial = ChocoGL::MaterialInstance::Create(ChocoGL::Material::Create(m_GridShader));
		m_GridMaterial->Set("u_Scale", m_GridScale);
		m_GridMaterial->Set("u_Res", m_GridSize);

		// Editor
		m_CheckerboardTex.reset(ChocoGL::Texture2D::Create("assets/editor/Checkerboard.tga"));

		// Environment
		m_EnvironmentCubeMap.reset(ChocoGL::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Radiance.tga"));
		//m_EnvironmentCubeMap.reset(ChocoGL::TextureCube::Create("assets/textures/environments/DebugCubeMap.tga"));
		m_EnvironmentIrradiance.reset(ChocoGL::TextureCube::Create("assets/textures/environments/Arches_E_PineTree_Irradiance.tga"));
		m_BRDFLUT.reset(ChocoGL::Texture2D::Create("assets/textures/BRDF_LUT.tga"));

		m_Framebuffer.reset(ChocoGL::Framebuffer::Create(1280, 720, ChocoGL::FramebufferFormat::RGBA16F));
		m_FinalPresentBuffer.reset(ChocoGL::Framebuffer::Create(1280, 720, ChocoGL::FramebufferFormat::RGBA8));
		...
		for (int i = 0; i < 8; i++)
		{
			ChocoGL::Ref<ChocoGL::MaterialInstance> mi(new ChocoGL::MaterialInstance(m_SphereMesh->GetMaterial()));
		...
		m_VertexBuffer.reset(ChocoGL::VertexBuffer::Create());
		m_VertexBuffer->SetData(data, 4 * sizeof(QuadVertex));

		uint32_t* indices = new uint32_t[6]{ 0, 1, 2, 2, 3, 0, };
		m_IndexBuffer.reset(ChocoGL::IndexBuffer::Create());
	...
	virtual void OnImGuiRender() override
	{
		...
		ImGui::Separator();
		{
			...
			{
				std::string filename = ChocoGL::Application::Get().OpenFile("");
				if (filename != "")
				{
					m_Mesh.reset(new ChocoGL::Mesh(filename));
					m_MeshMaterial.reset(new ChocoGL::MaterialInstance(m_Mesh->GetMaterial()));
				}
			}
		}
			...
			if (ImGui::CollapsingHeader("Albedo", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = ChocoGL::Application::Get().OpenFile("");
						if (filename != "")
							m_AlbedoInput.TextureMap.reset(ChocoGL::Texture2D::Create(filename, m_AlbedoInput.SRGB));
					}
				}
				ImGui::SameLine();
				ImGui::BeginGroup();
				...
				{
					if (m_AlbedoInput.TextureMap)
						m_AlbedoInput.TextureMap.reset(ChocoGL::Texture2D::Create(m_AlbedoInput.TextureMap->GetPath(), m_AlbedoInput.SRGB));
				}
				ImGui::EndGroup();
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Normals", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = ChocoGL::Application::Get().OpenFile("");
						if (filename != "")
							m_NormalInput.TextureMap.reset(ChocoGL::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Metalness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = ChocoGL::Application::Get().OpenFile("");
						if (filename != "")
							m_MetalnessInput.TextureMap.reset(ChocoGL::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
			...
			if (ImGui::CollapsingHeader("Roughness", nullptr, ImGuiTreeNodeFlags_DefaultOpen))
			{
				...
				if (ImGui::IsItemHovered())
				{
					...
					if (ImGui::IsItemClicked())
					{
						std::string filename = ChocoGL::Application::Get().OpenFile("");
						if (filename != "")
							m_RoughnessInput.TextureMap.reset(ChocoGL::Texture2D::Create(filename));
					}
				}
				ImGui::SameLine();
		...
		{
			auto& shaders = ChocoGL::Shader::s_AllShaders;
		...
		auto [wx, wy] = ChocoGL::Application::Get().GetWindow().GetWindowPos();
		posX -= wx;
		posY -= wy;
	...
	virtual void OnEvent(ChocoGL::Event& event) override
	{
	}
private:
	ChocoGL::Ref<ChocoGL::Shader> m_QuadShader;
	ChocoGL::Ref<ChocoGL::Shader> m_HDRShader;
	ChocoGL::Ref<ChocoGL::Shader> m_GridShader;
	ChocoGL::Ref<ChocoGL::Mesh> m_Mesh;
	ChocoGL::Ref<ChocoGL::Mesh> m_SphereMesh, m_PlaneMesh;
	ChocoGL::Ref<ChocoGL::Texture2D> m_BRDFLUT;

	ChocoGL::Ref<ChocoGL::MaterialInstance> m_MeshMaterial;
	ChocoGL::Ref<ChocoGL::MaterialInstance> m_GridMaterial;
	std::vector<ChocoGL::Ref<ChocoGL::MaterialInstance>> m_MetalSphereMaterialInstances;
	std::vector<ChocoGL::Ref<ChocoGL::MaterialInstance>> m_DielectricSphereMaterialInstances;
	...
	struct AlbedoInput
	{
		...
		ChocoGL::Ref<ChocoGL::Texture2D> TextureMap;
		bool SRGB = true;
	...
	std::unique_ptr<ChocoGL::Framebuffer> m_Framebuffer, m_FinalPresentBuffer;

	ChocoGL::Ref<ChocoGL::VertexBuffer> m_VertexBuffer;
	ChocoGL::Ref<ChocoGL::IndexBuffer> m_IndexBuffer;
	ChocoGL::Ref<ChocoGL::TextureCube> m_EnvironmentCubeMap, m_EnvironmentIrradiance;

	ChocoGL::Camera m_Camera;
	...
	// Editor resources
	ChocoGL::Ref<ChocoGL::Texture2D> m_CheckerboardTex;
};
