...
namespace ChocoGL {
		...
		: m_Format(format), m_Width(width), m_Height(height), m_Wrap(wrap)
	{
		...
		CL_RENDER_1(self, {
			...
			glTexImage2D(GL_TEXTURE_2D, 0, ChocoGLToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, ChocoGLToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, nullptr);
		...
		: m_FilePath(path)
	{
		...
		CL_RENDER_S1(srgb, {
			...
			else
			{
				...
				glTexImage2D(GL_TEXTURE_2D, 0, ChocoGLToOpenGLTextureFormat(self->m_Format), self->m_Width, self->m_Height, 0, srgb ? GL_SRGB8 : ChocoGLToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
				glGenerateMipmap(GL_TEXTURE_2D);
	...
	void OpenGLTexture2D::Unlock()
	{
		m_Locked = false;
		CL_RENDER_S({
			glTextureSubImage2D(self->m_RendererID, 0, 0, 0, self->m_Width, self->m_Height, ChocoGLToOpenGLTextureFormat(self->m_Format), GL_UNSIGNED_BYTE, self->m_ImageData.Data);
			});
	}
		...
		: m_FilePath(path)
	{
		...
		CL_RENDER_S3(faces, faceWidth, faceHeight, {
			...
			auto format = ChocoGLToOpenGLTextureFormat(self->m_Format);
