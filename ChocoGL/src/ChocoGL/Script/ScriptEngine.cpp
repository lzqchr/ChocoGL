...
namespace ChocoGL {
	...
	static void LoadChocoGLRuntimeAssembly(const std::string& path)
	{
		if (s_AppAssembly)
		{
	...
	{
		CL_CORE_ASSERT(s_EntityInstanceMap.find(entityID) != s_EntityInstanceMap.end(), "Could not find entity in instance map!");
	...
	void ScriptEngine::OnInitEntity(ScriptComponent& script, uint32_t entityID, uint32_t sceneID)
	{
		...
		// Retrieve public fields
		{
			...
			while ((iter = mono_class_get_fields(scriptClass.Class, &ptr)) != NULL)
			{
				...
				FieldType hazelFieldType = GetChocoGLFieldType(fieldType);
	...
	const ChocoGL::ScriptModuleFieldMap& ScriptEngine::GetFieldMap()
	{
