...
namespace ChocoGL {
	...
	{
		CL_CORE_ASSERT(Handle, "Entity has not been instantiated!");
	...
	static void InitMono()
	{
		...
		auto domain = mono_jit_init("ChocoGL");

		char* name = (char*)"ChocoGLRuntime";
	...
	{
		LoadChocoGLRuntimeAssembly(path);
	...
	void ScriptEngine::Init(const std::string& assemblyPath)
	{
		...
		LoadChocoGLRuntimeAssembly(s_AssemblyPath);
	}
	...
	{
		CL_CORE_ASSERT(s_EntityInstanceMap.find(dst) != s_EntityInstanceMap.end());
		CL_CORE_ASSERT(s_EntityInstanceMap.find(src) != s_EntityInstanceMap.end());
		...
		for (auto& [entityID, entityInstanceData] : srcEntityMap)
		{
			for (auto& [moduleName, srcFieldMap] : srcEntityMap[entityID].ModuleFieldMap)
			{
				...
				{
					CL_CORE_ASSERT(dstModuleFieldMap.find(moduleName) != dstModuleFieldMap.end());
					auto& fieldMap = dstModuleFieldMap.at(moduleName);
					CL_CORE_ASSERT(fieldMap.find(fieldName) != fieldMap.end());
	...
	void ScriptEngine::OnScriptComponentDestroyed(UUID sceneID, UUID entityID)
	{
		...
		CL_CORE_ASSERT(entityMap.find(entityID) != entityMap.end());
	...
	static FieldType GetChocoGLFieldType(MonoType* monoType)
	{
		...
		switch (type)
		{
			...
			case MONO_TYPE_VALUETYPE:
			{
				...
				if (strcmp(name, "ChocoGL.Vector2") == 0) return FieldType::Vec2;
				if (strcmp(name, "ChocoGL.Vector3") == 0) return FieldType::Vec3;
				if (strcmp(name, "ChocoGL.Vector4") == 0) return FieldType::Vec4;
			}
		}
	...
	void ScriptEngine::InitScriptEntity(Entity entity)
	{
		...
		{
			CL_CORE_ERROR("Entity references non-existent script module '{0}'", moduleName);
			return;
		}
	...
	{
		CL_CORE_ASSERT(s_EntityInstanceMap.find(sceneID) != s_EntityInstanceMap.end(), "Invalid scene ID!");
		auto& entityIDMap = s_EntityInstanceMap.at(sceneID);
		CL_CORE_ASSERT(entityIDMap.find(entityID) != entityIDMap.end(), "Invalid entity ID!");
	...
	static uint32_t GetFieldSize(FieldType type)
	{
		...
		CL_CORE_ASSERT(false, "Unknown field type!");
		return 0;
	}
	...
	{
		CL_CORE_ASSERT(m_EntityInstance->GetInstance());
