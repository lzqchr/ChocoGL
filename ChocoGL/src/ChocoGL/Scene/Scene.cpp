...
namespace ChocoGL {
	...
	static void OnScriptComponentConstruct(entt::registry& registry, entt::entity entity)
	{
		...
		CL_CORE_ASSERT(scene->m_EntityIDMap.find(entityID) != scene->m_EntityIDMap.end());
		...
		: m_DebugName(debugName)
	{
		...
		CL_CORE_ASSERT(physxWorld.World);
	...
	void Scene::OnRenderRuntime(Timestep ts)
	{
		...
		CL_CORE_ASSERT(cameraEntity, "Scene does not contain any cameras!");
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBody2DComponent>())
				{
					...
					CL_CORE_ASSERT(rigidBody2D.RuntimeBody);
...

		{
			...
			for (auto entity : view)
			{
				...
				if (e.HasComponent<RigidBodyComponent>())
				{
					...
					CL_CORE_ASSERT(rigidBody.RuntimeActor);
	...
	Entity Scene::CreateEntityWithID(UUID uuid, const std::string& name, bool runtimeMap)
	{
		...
		CL_CORE_ASSERT(m_EntityIDMap.find(uuid) == m_EntityIDMap.end());
